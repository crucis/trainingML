...
wds_model = net:clone():double()
wds = wds_model:getParameters()
wds:fill(opt.weightDecay)
for k,v in pairs(wds_model:findModules'nn.Mul') do
    v.weight:fill(0)
end
opt.weightDecay = 0
opt.weightDecays = wds
...
local function log(t)
    wds = opt.weightDecays
    opt.weightDecays = 0.0005
